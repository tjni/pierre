import type {
  ...
  PathStoreTreesViewportMetrics,
} from './types';

export const PATH_STORE_TREES_DEFAULT_ITEM_HEIGHT = 30;
export const PATH_STORE_TREES_DEFAULT_OVERSCAN = 10;
export const PATH_STORE_TREES_DEFAULT_VIEWPORT_HEIGHT = 420;
export const EMPTY_RANGE: PathStoreTreesRange = { start: 0, end: -1 };

function normalizeRange(
  range: PathStoreTreesRange,
  itemCount: number
): PathStoreTreesRange {
...
export function rangesEqual(
  left: PathStoreTreesRange,
  right: PathStoreTreesRange
): boolean {
...
export function computeVisibleRange({
  ...
  scrollTop,
  viewportHeight,
}: PathStoreTreesViewportMetrics): PathStoreTreesRange {
  if (itemCount <= 0) {
    return EMPTY_RANGE;
  }
...
export function computeWindowRange(
  metrics: PathStoreTreesViewportMetrics,
  currentRange: PathStoreTreesRange = EMPTY_RANGE
): PathStoreTreesRange {
  ...
  return expandRange(
    visibleRange,
    metrics.itemCount,
    metrics.overscan ?? PATH_STORE_TREES_DEFAULT_OVERSCAN
  );
}
